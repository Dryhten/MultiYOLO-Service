...
- Python >= 3.12
- PyTorch >= 2.0.0（根据硬件选择对应版本）
  - CPU: `torch`
  - CUDA: `torch` (带 CUDA 支持)
  - MUSA: `torch-musa`（参考摩尔线程官方文档）
...
docker run -d \
  -p 8003:8003 \
  -e YOLO_SERVICE_PORT=8003 \
  -e YOLO_SERVICE_MODEL_DIR=/app/module \
  -v $(pwd)/module:/app/module \
  --name yolo-service \
  yolo-service
```
...
更多 Docker 使用说明请参考 [DOCKER_USAGE.md](./DOCKER_USAGE.md)
